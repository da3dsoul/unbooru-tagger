...
public class DetectCommandHandlerTests
{
    ...
    /// which then serializes as a JSON write failure downstream in this file's tests,
    /// through no fault of DetectCommandHandler itself. Fixed, varying-but-bounded-away-
    /// from-zero values sidestep the cliff entirely.
    /// </summary>
    ...
    private static string CreateTestImage(int size)
    {
        ...
        // pre-normalization variance to exactly (or near) zero — the same precision
        // cliff FillDeterministic guards against on the weights side, so the input
        // shouldn't reintroduce it from the other direction.
